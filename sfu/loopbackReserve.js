...
exports.run = async (io) => {
  io.of(/^\/dynamic-\d+$/).on("connection", function (socket) {
    ...
    socket.on("_error", function (err) {
      console.error("socket ERROR:", err);
    });
    socket.on("_connect_error", (err) => {
      console.error("client connection error", err);
    });

    socket.on("_getRouterRtpCapabilities", (data, callback) => {
      if (router) {
    ...
    // --- producer ----
    socket.on("_createProducerTransport", async (data, callback) => {
    ...
    socket.on("_connectProducerTransport", async (data, callback) => {
      await producerTransport.connect({ dtlsParameters: data.dtlsParameters });
      sendResponse({}, callback);
    });

    socket.on("_produce", async (data, callback) => {
      ...
      newNamespace.emit("_newProducer", { kind: kind });
      if (consumerTransport) {
        console.log("-- emit newProducer --");
        newNamespace.emit("_newProducer", { kind: kind }); // send back too
      } else {
    ...
    // --- consumer ----
    socket.on("_createConsumerTransport", async (data, callback) => {
    ...
    socket.on("_connectConsumerTransport", async (data, callback) => {
      console.log("-- connectConsumerTransport ---");
      await consumerTransport.connect({ dtlsParameters: data.dtlsParameters });
      sendResponse({}, callback);
    });

    socket.on("_consume", async (data, callback) => {
    ...
    socket.on("_resume", async (data, callback) => {
      const kind = data.kind;
...
//       // inform clients about new producer
//       // console.log("--broadcast newProducer -- kind=", kind);
//       socket.broadcast.emit("_newProducer", { kind: kind });
//       if (consumerTransport) {
//         console.log("-- emit newProducer --");
//         socket.emit("_newProducer", { kind: kind }); // send back too
//       } else {
//         console.log("consumerTransport is NULL:", consumerTransport);
//       }
//     });
